...
O projeto inclui `vercel.json` e `api/index.ts` para publicar o frontend Vite e
o backend Express no mesmo domínio. Antes de implantar, cadastre
`GEMINI_API_KEY` nas variáveis do ambiente de Produção da Vercel. Depois de
alterar a variável, crie uma nova implantação para que o valor seja aplicado.

Após a implantação, abra `/api/health` no domínio do projeto. O resultado deve
mostrar `"status":"ok"`; com a chave configurada, `"hasApiKey"` deve ser
`true`.
